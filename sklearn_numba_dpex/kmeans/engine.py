...
class KMeansEngine(KMeansCythonEngine):
    ...
    def __init__(self, estimator):
        ...
        _testing_mode = os.getenv("SKLEARN_NUMBA_DPEX_TESTING_MODE", "0")
        if _testing_mode not in {"0", "1"}:
            raise ValueError(
                "If the environment variable SKLEARN_NUMBA_DPEX_TESTING_MODE is set, "
                f'it is expected to take values in {"0", "1"}, but got {_testing_mode}'
                " instead."
            )
        self._testing_mode = _testing_mode == "1"
    ...
    def prepare_fit(self, X, y=None, sample_weight=None):
        ...
        if self._testing_mode and X_mean is not None:
